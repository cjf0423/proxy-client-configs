 ...
 * ipaTool Web 操作面板
 * 作者: 小H
 * 说明: 拦截 https://apple-api.com/ui 请求，返回一个可视化操作网页
 *       在 Safari 中打开 https://apple-api.com/ui 即可使用
 *       所有 API 调用走 apple-api.com（由原始 AppleStoreAPI.js 处理）
 */
...
.field input,.field select{width:100%;padding:10px 12px;background:var(--bg);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:15px;outline:none}
.field input:focus,.field select:focus{border-color:var(--blue)}
.field input::placeholder{color:#484f58}
.btn-row{display:flex;gap:8px;flex-wrap:wrap}
.btn{flex:1;min-width:0;padding:10px 0;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;transition:opacity .2s}
...
.app-item{background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:12px;margin-bottom:8px;cursor:pointer;transition:border-color .2s}
...
.toast{position:fixed;top:60px;left:50%;transform:translateX(-50%);padding:10px 20px;border-radius:8px;font-size:14px;z-index:999;opacity:0;transition:opacity .3s;pointer-events:none}
...
.tab{flex:1;text-align:center;padding:8px 0;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer;color:var(--text2);transition:all .2s}
...
async function doRefresh() {
  ...
  try {
    ...
    setResult(res);
    toast(res.success ? 'Token 已刷新 ✅' : '刷新失败', res.success ? 'success' : 'error');
  ...
  try {
    ...
    if (res.success && res.data?.results?.length) {
      ...
      box.innerHTML = apps.map(app => {
        const name = app.trackName || app.name || '?';
        ...
        return '<div class="app-item" onclick="selectApp(\\'' + id + '\\',\\'' + name.replace(/'/g,"\\\\'") + '\\')">' +
...
async function doVersions() {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (Array.isArray(list) && list.length) {
        box.innerHTML = list.slice(0, 50).map(v => {
          ...
          return '<div class="ver-item">' +
            ...
            '</div>' +
            '<button class="btn btn-green" onclick="doDownload(\\'' + appId + '\\',\\'' + vid + '\\')">下载</button>' +
          '</div>';
        }).join('');
      ...
      box.innerHTML = '<div class="card" style="text-align:center;color:var(--red)">' + (res.message || '查询失败') + '</div>';
      toast('查询失败', 'error');
    }
  } catch(e) {
    box.innerHTML = '';
...
async function doDownload(appId, verId) {
  appId = appId || document.getElementById('appId').value.trim();
  if (!appId) return toast('请填写应用 ID', 'error');
...
async function doPurchase() {
  ...
  try {
    ...
    setResult(res);
    toast(res.success ? '购买成功 ✅' : '购买失败: ' + (res.message || ''), res.success ? 'success' : 'error');
...
// Surge/Stash 脚本：拦截 /ui 路径，返回 HTML 页面
...
if (url.includes('/ui')) {
  ...
  if (typeof $environment !== 'undefined' || typeof $httpClient !== 'undefined') {
    $done({ response });
  } else {
    $done({ status: 'HTTP/1.1 200 OK', headers: response.headers, body: HTML });
  }
} else {
  $done({});
}
